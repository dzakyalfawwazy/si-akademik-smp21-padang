  ...
  if (isset($_POST['simpan'])) {
   $query=$con->query("INSERT INTO tbl_kd values(NULL,'$_GET[kd_kelas]','$_GET[mapel]','Ulangan Harian','$_POST[kd]')");
   $query2=$con->query("INSERT INTO tbl_kd values(NULL,'$_GET[kd_kelas]','$_GET[mapel]','MID','$_POST[kd]')");
   $query3=$con->query("INSERT INTO tbl_kd values(NULL,'$_GET[kd_kelas]','$_GET[mapel]','Semester','$_POST[kd]')");
   if($query){
    echo "<script>alert('Data KD Berhasil Diinputkan!')</script>";
 ...
 <div class="row">
  <div class="col-md-4">
    ...
    <div class="card card-primary">
      <div class="card-body">
        ...
        <form method="post">
          <div class="form-group">
            <label for="tema">KD</label>
            <input type="text" class="form-control" <?= $dis ?> id="tema" name="kd">
          </div>
          <div class="form-group">
  ...
  <div class="card">
    <div class="card-header">
      <div class="text-green text-bold">Data KD</div>
    </div>
    <div class="card-body">
      <table class="table table-bordered table-striped" id="example3">
        <thead>
        <tr>
          <th>No.</th><th>Kelas</th><th>Mata Pelajaran</th><th>KD</th><th>Aksi</th>
        </tr>
        </thead>
        <?php @$qtema=$con->query("SELECT * from tbl_kelas, tbl_mapel, tbl_kd, tbl_pelajaran where tbl_kelas.kd_kelas=tbl_kd.kd_kelas and tbl_kd.kd_mapel=tbl_mapel.kd_mapel and tbl_mapel.kd_mapel='".$_GET['mapel']."' and tbl_pelajaran.kd_pel=tbl_mapel.kd_pel group by tbl_kd.kd_mapel,tbl_kd.kd");
        $no=0;
        ...
        <tr>
          ...
          <td><?= $row['namapel'] ?></td>
          <td><?= $row['kd'] ?></td>
          <td> <a class="btn btn-xs btn-danger" onclick="return confirm('Yakin menghapus?')" href="deleteguru.php?aksi=deletekd&id=<?=$row['id_kd']?>"><i class="fas fa-trash"></i></a></td>
        </tr>
        <?php } ?>
      </table>
    </div>
