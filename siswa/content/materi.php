...
<table class="table">
          ...
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
              <?php
               @$qukel=mysqli_query($con,"select * from tbl_materi join tbl_mapel on tbl_materi.kd_mapel=tbl_mapel.kd_mapel join tbl_pelajaran on tbl_mapel.kd_pel=tbl_pelajaran.kd_pel where tbl_materi.kd_kelas='$_GET[kd_kelas]' and tbl_materi.kd_mapel='$_GET[mapel]'");
              $no=0;
              while($dakel=mysqli_fetch_array($qukel)){
                ...
                <td><?= $dakel['namapel'] ?></td>
                <td><?= $dakel['judul'] ?></td>
                <td><a class="btn btn-sm btn-success" href="index.php?p=8&id_materi=<?= $dakel['id_materi'] ?>">Lihat lebih lanjut</a> </td>
              </tr>
              <?php } ?>
            </tbody>
          </table>
